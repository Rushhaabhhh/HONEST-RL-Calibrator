...
from server.generators.code_gen import generate
...
def test_returns_valid_tuple(difficulty):
    ...
    assert "Given this Python function:" in question
    assert "what does f(" in question
...
def test_different_seeds_vary(difficulty):
    outputs = {generate(difficulty, seed=s) for s in range(15)}
